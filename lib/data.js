...
export async function getAnimeWithThemes(id) {
  const { data, error } = await sb
    .from('anime')
    .select(
      'id, title, title_romaji, season, year, cover_image_url, format, synopsis, studio, themes(id, theme_type, sequence_number, title, artist, release_season, release_year, theme_variants(id, quality, source, url))'
    )
    .eq('id', id)
    .maybeSingle();
...
export async function getRankableThemes(themeType) {
  const { data, error } = await sb
    .from('themes')
    .select(
      'id, title, artist, sequence_number, release_season, release_year, anime:anime_id(id, title), theme_variants(id, quality, source, url)'
    )
...
export async function getThemeWithAnime(id) {
  const { data, error } = await sb
    .from('themes')
    .select(
      'id, theme_type, sequence_number, title, artist, anime:anime_id(id, title), theme_variants(id, quality, source, url)'
    )
    .eq('id', id)
    .maybeSingle();
